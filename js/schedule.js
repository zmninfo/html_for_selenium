  ...
  function Schedule(opt) {
    let render = function () {
      ...
      for (let i = 0; i < total; i++) {
        if (i < startWeek) {
          eleTemp.push('<li class="other-month"><span class="dayStyle">'
            + (lastMonthDay - startWeek + 1 + i) + '</span></li>')
          ...
          eleTemp.push('<li class="current-month" ><span title=' + nowDate
            + ' class="currentDate dayStyle ' + addClass + '">' + (i + 1
              - startWeek) + '</span></li>')
        } else {
          eleTemp.push('<li class="other-month"><span class="dayStyle">' + (i
            + 1 - (startWeek + fullDay)) + '</span></li>')
        }
      }
    ...
    const init = function () {
      const scheduleHd = '<div class="schedule-hd">' +
        ...
        +
        '</div>' +
        '<div class="today">' + formartDate(year, month + 1, day, '-')
        + '</div>' +
        '<div>' +
